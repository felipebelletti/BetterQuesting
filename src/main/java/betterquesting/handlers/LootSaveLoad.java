...
public class LootSaveLoad {
    ...
    public void LoadLoot(MinecraftServer server) {
        ...
        LootRegistry.INSTANCE.readFromNBT(NBTConverter.JSONtoNBT_Object(j1, new CompoundTag(), true), false);
    }

    public void SaveLoot() {
        JsonHelper.WriteToFile(new File(worldDir, "QuestLoot.json"), NBTConverter.NBTtoJSON_Compound(LootRegistry.INSTANCE.writeToNBT(new CompoundTag(), null), new JsonObject(), true));
    }
