...
public class FactorySlicedTexture implements IFactoryData<IGuiTexture, JsonObject> {
    ...
    public SlicedTexture loadFromData(JsonObject data) {
        ...
        return new SlicedTexture(atlas, new GuiRectangle(bounds[0], bounds[1], bounds[2], bounds[3]), new GuiPadding(padding[0], padding[1], padding[2], padding[3])).setSliceMode(SliceMode.values()[sliceMode]);
    }
